...
def _vllm_generate_sequences(vllm_model, prompt_token_ids, max_new_tokens, temperature, until=None):
    if SamplingParams is None:
        raise ModuleNotFoundError("vllm is required but not installed.")
    ...
    if until is not None:
        stop = [until] if isinstance(until, str) else until
        if stop:
    ...
    return [output.outputs[0].token_ids if output.outputs else [] for output in outputs]

def generate_candidate_blocks(model, block_size, m_candidates, tokenizer, current_tokens, temperature, vllm_model=None):
    if vllm_model is not None:
        prompt_token_ids = [current_tokens[0].tolist() for _ in range(m_candidates)]
        generated_token_ids_list = _vllm_generate_sequences(
        ...
        return [
            torch.tensor(token_ids, device=current_tokens.device, dtype=current_tokens.dtype)
...
def importance_sampling_guidance(
    ...
    ##list [m_candidates, response_length]
    candidate_blocks = generate_candidate_blocks(
        model, block_size, m_candidates, tokenizer, current_tokens, temperature, vllm_model
    )
    # breakpoint()
    ...
    for block_idx, block_tokens in enumerate(candidate_blocks):
        candidate_sequence = torch.cat([
            current_tokens[0], #req [1, 1041]
            block_tokens #rep [128]
        ]).unsqueeze(0)
    ...
    if batched_inputs: 
        ...
        if use_importance_sampling:
            ...
            prompt_token_ids = [token_ids.tolist() for m_tokens in batched_inputs for token_ids in m_tokens]
            generated_token_ids_list = _vllm_generate_sequences(
                vllm_model=vllm_model,
                prompt_token_ids=prompt_token_ids,
                max_new_tokens=max_new_tokens,
            ...
            all_full_sequences = [
                ...
                for prompt_ids, generated_ids in zip(prompt_token_ids, generated_token_ids_list)
            ]
        else:
            all_inputs = torch.cat(batched_inputs, dim=0)
...
def generate(
    ...
    current_tokens = input_ids #[1, 1041]
    all_candidate_answers = []
    ...
    for block_step in range(total_blocks):
        ...
        next_block, all_candidate_answers = importance_sampling_guidance(
            ...
            until, all_candidate_answers, use_importance_sampling, small_model, vllm_model
        )
        ...
        if next_block is not None:
            ...
            if vllm_model is not None and not use_importance_sampling:
                generated_token_ids = _vllm_generate_sequences(
                    vllm_model=vllm_model,
                    prompt_token_ids=[current_tokens[0].tolist()],
                    max_new_tokens=max_new_tokens,
                    temperature=temperature,
                    until=until,
                )[0]
